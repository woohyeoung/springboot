...
public class UserController {
	...
	public ResponseDTO login(@RequestBody UserSignInRequestDTO userSignInRequestDTO) {
		...
		if(result.getKey() == 0) return new ResponseDTO().fail(HttpStatus.BAD_REQUEST, Payload.SIGN_IN_FAIL + result.getMessage());
	...
	public ResponseDTO logout(@RequestBody UserRequestDTO userRequestDTO) {
		...
		UserSignInResponseDTO result = userService.logout(userRequestDTO);
